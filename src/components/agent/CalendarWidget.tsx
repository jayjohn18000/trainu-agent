...
export function CalendarWidget({
  ...
  const timeUntil = nextSession ? formatDistanceToNow(nextSession.time, {
    addSuffix: true
  }) : null;
  if (isLoading) {
    return <Card>
  ...
  return <Card>
      ...
      <CardContent className="space-y-3">
        {nextSession && <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
        ...
        <div className="space-y-2">
          {sessions.slice(1).map(session => <div key={session.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer" onClick={onOpenCalendar}>
