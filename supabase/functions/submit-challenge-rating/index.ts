...
serve(async (req: Request) => {
  ...
  try {
    ...
    const submitRatingSchema = z.object({
      ...
      ratingExpertise: z.number().int().min(1).max(3, "Rating must be 1-3"),
      ratingCommunication: z.number().int().min(1).max(3, "Rating must be 1-3"),
      ratingMotivation: z.number().int().min(1).max(3, "Rating must be 1-3"),
      ratingResults: z.number().int().min(1).max(3, "Rating must be 1-3"),
      ratingValue: z.number().int().min(1).max(3, "Rating must be 1-3"),
